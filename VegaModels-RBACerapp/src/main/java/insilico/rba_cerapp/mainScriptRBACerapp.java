...
public class mainScriptRBACerapp {
    private static final Logger log = LogManager.getLogger(mainScriptRBACerapp.class);

    public static void main(String[] args) throws Exception {
        InsilicoModel model = new ismEstrogenBindingCerapp();
        ModelsDeployment.BuildDataset(model, "out_ts");
        File sourceFile = new File("out_ts/" + model.getInfo().getTrainingSetURL() + "/" + model.getInfo().getTrainingSetURL().split("/data/")[1]);
        File destinationFile = new File("VegaModels-RBACerapp\\src\\main\\resources\\data\\ts_estrogen_cerapp.dat");
        try {
