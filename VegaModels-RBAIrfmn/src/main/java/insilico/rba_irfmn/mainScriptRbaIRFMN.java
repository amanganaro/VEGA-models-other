...
public class mainScriptRbaIRFMN {
    private static final Logger log = LogManager.getLogger(mainScriptRbaIRFMN.class);

    public static void main(String[] args) throws Exception {

        InsilicoModel model = new ismRbaIRFMN();
        ModelsDeployment.BuildDataset(model, "out_ts");
        File sourceFile = new File("out_ts/" + model.getInfo().getTrainingSetURL() + "/" + model.getInfo().getTrainingSetURL().split("/data/")[1]);
        File destinationFile = new File("VegaModels-RBAIrfmn\\src\\main\\resources\\data\\ts_rba_irfmn.dat");
        try {
