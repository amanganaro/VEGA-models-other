...
public class mainScriptPGPNic {
    private static final Logger log = LogManager.getLogger(mainScriptPGPNic.class);

    public static void main(String[] args) throws Exception {

        InsilicoModel model = new ismPgpNic();
        ModelsDeployment.BuildDataset(model, "out_ts");
        File sourceFile = new File("out_ts/" + model.getInfo().getTrainingSetURL() + "/" + model.getInfo().getTrainingSetURL().split("/data/")[1]);
        File destinationFile = new File("VegaModels-PGPNic\\src\\main\\resources\\data\\ts_pgp_nic.dat");
        try {
